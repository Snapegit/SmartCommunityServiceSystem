...
<template>
	<div>
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.buynumberstart" placeholder="最小购买数量"
								clearable>
							</el-input>至
							<el-input class="search_inp" v-model="searchQuery.buynumberend" placeholder="最大购买数量"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					...
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('orders'+(orderStatus?'/'+orderStatus:''),'销售额统计')">销售额统计</el-button>
				</div>
			</div>
			<br>
			<el-table
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'销售额统计'" width="70%">
			<template #footer>
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.buynumberstart){
			params['buynumberstart'] = searchQuery.value.buynumberstart
		}
		if(searchQuery.value.buynumberend){
			params['buynumberend'] = searchQuery.value.buynumberend
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
