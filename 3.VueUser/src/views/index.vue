<template>
	<div class="home">
		<div class="index_top">
			<div class="index_top_title">基于Springcloud的智能社区服务系统</div>
		...
		<el-scrollbar class="contain_view">
			...
			<div class="rotation_view">
				<mySwiper :type="3" :data="rotationList" :autoHeight="false" :autoplay="true"
					...
					:freeMode="false" :effectType="10"
					:direction="horizontal">
					<template #default="scope">
						<img :style='{"border":"0px solid #fff","width":"100%","borderRadius":"4px","height":"410px"}' :src="scope.row.value?$config.url + scope.row.value:''">
					</template>
				</mySwiper>
			</div>
			<router-view />
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'yezhu'){
				context?.$toolUtil.storageSet("frontName", res.data.data.yezhuzhanghao)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'yezhu'){
			...
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'wuyerenyuan'){
				context?.$toolUtil.storageSet("frontName", res.data.data.wuyezhanghao)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'wuyerenyuan'){
				context?.$toolUtil.storageSet('headportrait',res.data.data.touxiang)
			}
...
<style lang="scss" scoped>
	...
	.index_top {
		...
		z-index: 1002;
		color: #fff;
		background: url(http://clfile.zggen.cn/20231020/98766058521b4a04a01a70fe9b14b6b4.jpg) repeat-x center top,#fff;
		display: block;
		width: 100%;
	...
	.contain_view {
		margin: 0;
		background: url(http://clfile.zggen.cn/20231020/98766058521b4a04a01a70fe9b14b6b4.jpg) inherit repeat-x center -140px,#fff;
		position: inherit;
		height: 100vh;
	}
	.el-aside {
	...
	.menu_scrollbar {
		...
		padding: 0;
		margin: 16px auto 0;
		background: url(http://clfile.zggen.cn/20231020/dd687dc1ffd944c5ad2c38d477f86ebc.jpg) repeat-x center top;
		width: 64%;
		border-color: #efefef;
	...
	.menu_view {
		...
		:deep(.is-active) {
			color: #fff !important;
			background: url(http://clfile.zggen.cn/20231020/211a1c936aff44009a17016e8cd0eeaf.jpg) no-repeat center top / 100% 100% !important;
