	...
	<div class="app-contain" :style='{"padding":"20px 0px","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"64%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">业主账号</div>
					<div  class="info_text" >{{detail.yezhuzhanghao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">业主姓名</div>
					<div  class="info_text" >{{detail.yezhuxingming}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">楼房号</div>
					<div  class="info_text" >{{detail.loufanghao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">维修类型</div>
					<div  class="info_text" >{{detail.weixiuleixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">报修时间</div>
					<div  class="info_text" >{{detail.baoxiushijian}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">维修状态</div>
					<div  class="info_text" >{{detail.weixiuzhuangtai}}</div>
				</div>
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'维修')" @click="weixiuxinxionAcross('维修','weixiuxinxi','','','')" type="warning">维修</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('weixiushenqing','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('weixiushenqing','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="报修内容" name="first">
				<div v-html="detail.baoxiuneirong"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const weixiuxinxionAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
