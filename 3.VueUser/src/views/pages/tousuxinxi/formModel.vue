	...
	<div class="app-contain" :style='{"padding":"20px 0px","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"64%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">投诉编号</div>
					<div  class="info_text" >{{detail.tousubianhao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">投诉时间</div>
					<div  class="info_text" >{{detail.tousushijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="edit_btn" v-if="centerType&&btnAuth('tousuxinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('tousuxinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.wuyezhanghao
