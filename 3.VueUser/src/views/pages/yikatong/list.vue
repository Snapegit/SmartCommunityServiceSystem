		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.yezhuxingming" placeholder="业主姓名"
						clearable>
					</el-input>
				</div>
			</div>
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('yikatong','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="卡号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kahao}}
							</template>
						</el-table-column>
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.yezhuxingming&&searchQuery.value.yezhuxingming!=''){
			params.yezhuxingming = '%' + searchQuery.value.yezhuxingming + '%'
		}
		context?.$http({
	...
	const tableDetailClick = (row) => {
		router.push('yikatongDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
